...
todo_config() ->
    #{<<"/">> => #{<<"GET">> => {todo_handler, get_todos}},
      <<"/todo">> =>
          [{nine_mid, urlencoded_params}, #{<<"POST">> => {todo_handler, post_todo}}],
      <<"/todo/:id">> => #{<<"GET">> => {todo_handler, get_todo}},
      <<"/api">> =>
          [#{<<"/todo/:id">> => #{<<"GET">> => {todo_handler, get_todo_json}},
             <<"/todo">> =>
                 [{nine_mid, json_request},
...
%% Example route configs used for testing
%% config6() ->
